...
# collision can happen on these directions, the ground, no collision and rotation
...
class CollisionDetector:
    ...
    def check_board(self, shape_coordinates):
        if np.max(shape_coordinates[:, 1]) >= self.board.height:
            return Collision.BOTTOM
        if np.max(shape_coordinates[:, 0]) >= self.board.width:
            return Collision.LEFT
        if np.min(shape_coordinates[:, 0]) < 0:
            return Collision.RIGHT
        return Collision.NONE

    #check collision with ground and other tiles
    def check_ground(self, tile_coordinates, dx, dy):
        ground_coordinates = self.ground.get_coordinates()
